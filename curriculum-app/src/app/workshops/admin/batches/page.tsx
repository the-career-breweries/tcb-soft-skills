    ...
    <div>
      <div className="admin-page-header">
        <h1 className="admin-page-title">
          {activeTab === 'batches' && 'Institutional Registrations'}
          {activeTab === 'registrations' && 'Individual Registrations'}
          {activeTab === 'builder' && 'Workshop Builder'}
          {activeTab === 'roster' && 'Student Roster & Reports'}
        </h1>
        ...
        {(activeTab === 'builder' || activeTab === 'roster') && (
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
            ...
            >
              <Users size={18} /> Reports & Certificates
            </button>
            <button 
              className="admin-btn" 
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          {isLoadingBatches ? (
            <div className="admin-card" style={{ textAlign: 'center', padding: '3rem' }}>
              <Loader2 className="animate-spin" size={24} style={{ margin: '0 auto 1rem', color: '#3b82f6' }} />
            ...
            .map(([days, groupBatches]) => (
              <div key={days} className="admin-table-container admin-card" style={{ padding: 0, overflow: 'hidden' }}>
                <div style={{ 
                  ...
                  {days}-Day Workshops
                  <span style={{ fontSize: '0.75rem', fontWeight: 500, backgroundColor: '#e2e8f0', color: '#475569', padding: '0.125rem 0.5rem', borderRadius: '9999px' }}>
                    {(groupBatches as any[]).length}
                  </span>
                </div>
          ...
          <table className="admin-table">
            ...
            <tbody>
              {isLoadingRegistrations ? (
                <tr><td colSpan={6} style={{ textAlign: 'center', padding: '2rem' }}><Loader2 className="animate-spin" /> Loading registrations...</td></tr>
              ) : registrations.length === 0 ? (
                <tr><td colSpan={6} style={{ textAlign: 'center', padding: '2rem', color: '#64748b' }}>No individual registrations yet.</td></tr>
              ) : (
