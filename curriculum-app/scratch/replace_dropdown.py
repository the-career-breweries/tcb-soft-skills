...
content = content.replace('export default function SoftSkillsApp() {', dropdown_component + '\nexport default function SoftSkillsApp() {')

prog_sel_regex = r'\{\/\* Program Selector \*\/\}.*?<\/select>'
prog_replacement = """{/* Program Selector */}
                  <CustomDropdown 
                    value={program} 
                  ...
                  />"""
content = re.sub(prog_sel_regex, prog_replacement, content, flags=re.DOTALL)

stream_sel_regex = r'\{\/\* Stream Selector \*\/\}.*?<\/select>'
stream_replacement = """{/* Stream Selector */}
                  <CustomDropdown 
                    value={selectedStream} 
                    onChange={handleStreamChange}
                    options={streams.map(s => ({ label: s.streamName, value: s.streamName }))}
                  />"""
content = re.sub(stream_sel_regex, stream_replacement, content, flags=re.DOTALL)

sem_sel_regex = r'\{\/\* Semester Selector \*\/\}.*?<\/select>'
sem_replacement = """{/* Semester Selector */}
                  <CustomDropdown 
                  ...
                  />"""
content = re.sub(sem_sel_regex, sem_replacement, content, flags=re.DOTALL)
